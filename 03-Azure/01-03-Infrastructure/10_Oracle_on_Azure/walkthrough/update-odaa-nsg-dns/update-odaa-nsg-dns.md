...
1. From the overview portal of the deployed ADB database, copy the FQDN of the "database private url" and Database private IP address both in the section Network.
...
5. After you press the Add button a new window will be opened where the name for the ADB and the private IP address needs to be added. In addition change the TTL to 10 seconds.
   ...
   > [!IMPORTANT]
   > **Use the cryptic hostname prefix, NOT your ADB display name!**
   > 
   > The "Database private URL" from the Azure Portal looks like this:
   > ```
   > t6bchxz9.adb.eu-paris-1.oraclecloud.com
   > ```
   > 
   > - ✅ **Correct:** Enter only the first part before the dot: `t6bchxz9` (this is the Oracle-generated unique identifier)
   > - ❌ **Wrong:** Do NOT use your chosen ADB display name (e.g., `myauto-adb-userXX`)
   > 
   > The cryptic prefix is automatically generated by Oracle and is the actual hostname that clients use to connect.
