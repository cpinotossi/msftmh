...
output "dns_zone_resource_group" {
  ...
  value       = var.create_dns_link ? (var.dns_zone_resource_group != null && trimspace(coalesce(var.dns_zone_resource_group, "")) != "" ? var.dns_zone_resource_group : azurerm_resource_group.vm.name) : null
}

output "dns_zone_id" {
  description = "Private DNS Zone ID (only when created in this module)"
  value       = var.create_dns_link && (var.dns_zone_resource_group == null || trimspace(coalesce(var.dns_zone_resource_group, "")) == "") ? azurerm_private_dns_zone.odaa[0].id : null
}
